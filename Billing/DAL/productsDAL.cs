...
namespace Billing.DAL
{
    class productsDAL
    {
        ...
        public bool Insert(productsBLL c)
        {
            ...
            try
            {
                string sql = "INSERT INTO tbl_products(name,category,description,rate,qty,min,added_date,added_by)VALUES(@name,@category,@description,@rate,@qty,@min,@added_date,@added_by)";
                SqlCommand cmd = new SqlCommand(sql, conn);
                ...
                cmd.Parameters.AddWithValue("@min", c.min);
        ...
        public bool Update(productsBLL p)
        {
            ...
            try
            {
                string sql = "UPDATE tbl_products SET name=@name,category=@category,description=@description,rate=@rate,min=@min,added_date=@added_date,added_by=@added_by WHERE Id=@Id";
                SqlCommand cmd = new SqlCommand(sql, conn);
        ...
        public productsBLL GetProductsFromTransaction(string keyword)
        {
            ...
            try
            {
                ...
                if (dt.Rows.Count > 0)
                {
                    ...
                    p.qty = int.Parse(dt.Rows[0]["qty"].ToString());  
                }
        ...
        #endregion
        #region getProductForPurchaseSales
        public productsBLL GetProductsForPurchaseSales(string keyword)
        {
            productsBLL p = new productsBLL();
            SqlConnection conn = new SqlConnection(myconnstring);
            DataTable dt = new DataTable();
            try
            {
                string sql = "SELECT name,rate,qty,min FROM tbl_products where name LIKE '%" + keyword + "%'";
                SqlDataAdapter adaptor = new SqlDataAdapter(sql, conn);
                conn.Open();
                adaptor.Fill(dt);
                if (dt.Rows.Count > 0)
                {
                    ...
                    p.rate = decimal.Parse(dt.Rows[0]["rate"].ToString());
                    p.qty = int.Parse(dt.Rows[0]["qty"].ToString());
                    p.min =int.Parse( dt.Rows[0]["min"].ToString());
                }
